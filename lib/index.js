...
function appPaths(options) {
    ...
    return {
        ...
        pathTo: function pathTo(dir) {
            var fullpath = false;
            if (dirs && dirs[dir]) {
                // The dir is the the user options. Return the predefined path
                fullpath = path.resolve(__dirname, toRoot, dirs[dir]);
            } else {
                // The dir is not in the user options. Return the full path
                fullpath = path.join(__dirname, toRoot, dir);
            }
